...
import com.example.specialmps.presentation.hospital.HospitalActivity
...
//        }
        for (i in 1..4) {//10월 28일
            colorArray.add(carray[3])
        }
        for (i in 1..5) {//11월 4일
            colorArray.add(carray[1])
        }
        for (i in 1..6) {//11월 11일
            colorArray.add(carray[3])
        }
        for (i in 1..13) {//11월 13일 ->원래 13개
            colorArray.add(carray[0])
        }
        for (i in 1..8) {//11월 14일
            colorArray.add(carray[2])
        }
        colorArray.add(carray[2])//11월 15일
        for (i in 1..5) {//12월 4일
            colorArray.add(carray[2])
        }
        for (i in 1..8) {//12월 5일 안들어 간거
            colorArray.add(carray[2])
        }
        CoroutineScope(Dispatchers.IO).launch {
...
//        }
    ...
    private fun findColor(day: CalendarDay?): String {
        ...
        if (day!!.day in 1..9) {
            check =
                day!!.year.toString() + "-" + (day!!.month + 1).toString() + "-0" + day!!.day.toString()
        } else {
            check =
                day!!.year.toString() + "-" + (day!!.month + 1).toString() + "-" + day!!.day.toString()
        }
